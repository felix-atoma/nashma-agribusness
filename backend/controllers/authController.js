...
// Google authentication fallback for development
exports.googleAuthDev = catchAsync(async (req, res, next) => {
  ...
  if (!email) {
    return next(new AppError('Email is required for development Google auth', 400));
  }
  ...
  if (!user) {
    ...
    user = await User.create({
      ...
      password: 'dev-password-' + Date.now(),
      passwordConfirm: 'dev-password-' + Date.now(),
      isGoogleAuth: true,
    });
  }
